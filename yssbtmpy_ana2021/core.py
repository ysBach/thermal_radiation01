...
class SmallBody():
    ...
    def set_ecl(self, r_hel, hel_ecl_lon, hel_ecl_lat,
        ...
        r_hel, r_obs : float, Quantity
            The heliocentric and observer-centric distance to the object (in au if `float`).

        hel_ecl_lon, hel_ecl_lat, obs_ecl_lon, obs_ecl_lat : float, Quantity
            The heliocentric ecliptic and observer-centric ecliptic longitude and latitude (in degrees
            if `float`).

        alpha : float, Quantity
            The phase angle (Sun-target-observer angle) (in degrees if `float`). It is not trivial to
            know `alpha` from ecliptic longitudes and latitudes, because the observer is likely not at
            the heliocentric distance of 1 au (i.e., geocenter).
    ...
    def set_spin(self, spin_ecl_lon, spin_ecl_lat, rot_period):
        ...
        spin_ecl_lon, spin_ecl_lat : float, Quantity
            The ecliptic longitude and latitude of the spin vector (in degrees if `float`).
        ...
        try:
            vec = lonlat2cart(lon=self.spin_ecl_lon, lat=self.spin_ecl_lat, r=1)
    ...
    def set_mass(self, diam_eff=None, mass=None, bulk_mass_den=None):
        ...
        diam_eff : float, Quantity, optional.
            The effective diameter (in meters if `float`). It might be overriden by `diam_eff` of
            `.set_optical()`, so be sure they are identical in the script.
    ...
    def set_optical(self, hmag_vis=None, slope_par=None, diam_eff=None, p_vis=None, a_bond=None,
        ...
        hmag_vis, slope_par : float, optional.
            The absolute magnitude and slope parameter in the V-band, following the IAU H-G magnitude
            system (Bowell et al. 1989).

        diam_eff : float, Quantity, optional.
            The effective diameter (in meters if `float`). It might be overriden by `diam_eff` of
            `.set_mass()`, so be sure they are identical in the script.

        p_vis : float, optional.
            The geometric albedo in the V-band.

        a_bond : float, optional.
            The Bond albedo. If not given, calculated based on the IAU H-G magnitude system, assuming
            ``A_Bond ~ A_V`` (Bowell et al. 1989).

        phase_int : float, optional.
            The phase integral. If not given, calculated based on the IAU H-G magnitude system (Bowell
            et al. 1989).
        '''
    ...
    def set_thermal(self, ti, emissivity, eta_beam=1):
        ...
        emissivity : float
            The spectrum-averaged hemispherical emissivity of the surface material (assumption is that
            directional emissivity is direction- and wavelength-independent).

        eta_beam : float, optional.
            The beaming parameter (eta) in the simple NEATM or STM, used as a first-order correction to
            the roughness effect in TPM (not useful for phase angle larger than ~ 40 degrees).
            Default is 1.0 (no beaming effect).
        '''
    ...
    def set_tpm(self, nlon=360, nlat=90, Zmax=10, dZ=0.2):
        ...
        Notes
        -----
        The number of slabs is defined by ``self.nZ = int(np.around(Zmax//dZ))``, so if ``Zmax`` is not
        an integer multiple of ``dZ``, say ``Zmax=10`` and ``dZ=0.15``, there will be
        ...
        if (self.dlon/(self.dZ)**2) > 0.5:
            raise ValueError(
                "dlon/dZ^2 > 0.5 !! The solution may not converge. Tune such that nlon > 4*PI/dZ^2 "
                + "( = {:.2f}).".format(4*PI/self.dZ**2)
            )
    ...
    def calc_temp(self, full=False, min_iter=50, permanent_shadow_u=0, in_kelvin=False, verbose=False):
        ...
        if verbose and self.thermal_par < 0.1:
            if self.nlon < 1000:
                print(f"nlon ({self.nlon}) is too small for such thermal parameter ({self.thermal_par:.3e}); "
                      + "recommended to be >= 1500.")
        ...
        self.mu_suns = calc_mu_vals(r_vec=self.r_hel_vec,
        ...
        self.spl_uarr = RectBivariateSpline(colats_spl, phases_spl, u_arr[:, :, 0], kx=1, ky=1, s=0)
    ...
    def tohdul(self, output=None, dtype='float32', **kwargs):
        ...
        for i, hdr in enumerate([hdu_0.header, hdu_T.header, hdu_m.header]):
            ...
            hdr["CRVAL2"] = (-90 + self.dlat/2*R2D, "Coordinate value at reference point")
